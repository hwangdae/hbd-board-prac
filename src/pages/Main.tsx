...
const Main: React.FC<any> = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const boardsData = await axios.get(`${process.env.REACT_APP_SERVER_URL}/boards`);
      setData(boardsData.data);
    } catch (error) {
  ...
  const handleBoardSubmit = async (e: any) => {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_SERVER_URL}/boards`,{email,contents,isDeleted:false})
        ...
        {data?.map((item: any, index) => (
          <ListItem key={item.id}>
            <span>
             ...
             {email == item.email ? <Button>삭제</Button> : ""}
          </ListItem>
        ))}
      </ListWrapper>
    </MainWrapper>
